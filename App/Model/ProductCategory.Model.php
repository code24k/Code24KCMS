...
class ProductCategoryModel {
    ...
    public function List(\XHB2\Request &$request){
        ...
        $sqlCount = "select count(Id) countTotal from ProductCategory where {$where};";
        ...
        $sqlresult = "select Id,Title,Status,CreateTime,UpdateTime from ProductCategory where {$where} order by CreateTime desc {$limit};";
    ...
    public function Add(\XHB2\Request &$request){
        ...
        $databaseManager = DatabaseManager::Instance();
        $fetchRow = $databaseManager->FetchRow("select Id,Translate from ProductCategory where Translate=:Translate and Title=:Title;", [
        ...
        $result = $databaseManager->Query("insert into ProductCategory (Id,Title,Keywords,Description,Status,CreateTime,UpdateTime,Translate) values (:Id,:Title,:Keywords,:Description,:Status,:CreateTime,:UpdateTime,:Translate);", [
            "Id" => CreateId(),
            "Title" => $param["Title"],
    ...
    public function Edit(\XHB2\Request &$request){
        ...
        $databaseManager = DatabaseManager::Instance();
        $updateTime = date("Y-m-d H:i:s");
        $result = $databaseManager->Query("update ProductCategory set Title=:Title,Keywords=:Keywords,Description=:Description,Status=:Status,UpdateTime=:UpdateTime where Translate=:Translate and Id=:Id;", [
            "Id" => $param["Id"],
            "Translate" => $request->GetTranslate(),
            "Title" => $param["Title"],
            "Keywords" => $param["Keywords"],
    ...
    public function Delete(\XHB2\Request &$request){
        ...
        $databaseManager = DatabaseManager::Instance();
        $result = $databaseManager->Query("delete from ProductCategory where Id=:Id;", [
            "Id" => $param["Id"],
        ]);
        if($result){
            return StatusSuccess(Translate($request->GetTranslate(), "StatusSuccess"));
    ...
    public function GetDetail(\XHB2\Request &$request, string $Id){
        $fetchRow = DatabaseManager::Instance()->FetchRow("select Id,Title,Keywords,Description,Status,CreateTime,UpdateTime,Translate from ProductCategory where Translate=:Translate and Id=:Id;", [
            "Translate" => $request->GetTranslate(),
            "Id" => $Id,
        ]);
        if($fetchRow == false){
    ...
    public function GetListArray(\XHB2\Request &$request){
        $listData = [];
        $sqlresult = "select Id,Title from ProductCategory where Status=1 order by CreateTime desc;";
